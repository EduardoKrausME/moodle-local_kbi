...
class block extends external_api {
    ...
    public static function sequence($elements) {
        ...
        foreach ($elements as $element) {
            if ($element) {
                ...
                $DB->update_record("local_kbi_block", $koperebiblock);
            }
        }
    ...
    public static function delete($blockid) {
        ...
        $koperebiblock = $DB->get_record("local_kbi_block", ["id" => $params["block_id"]]);
        if (!$koperebiblock) {
            return [
                "status" => "ERRO",
            ];
        ...
        $DB->delete_records("local_kbi_block", ["id" => $params["block_id"]]);
        $DB->delete_records("local_kbi_element", ["block_id" => $params["block_id"]]);
    ...
    public static function add($pageid, $type) {
        ...
        $koperebiblock->id = $DB->insert_record("local_kbi_block", $koperebiblock);
