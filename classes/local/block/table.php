...
class table implements i_type {
    ...
    public static function get_name() {
        return get_string("table_name", "local_kbi");
    }
    ...
    public static function get_description() {
        return get_string("table_desc", "local_kbi");
    }
    ...
    public function edit_columns(form $form, $koperebielement) {
        ...
        message::print_info(get_string("table_info_topo", "local_kbi"));
        if (isset($lines[0])) {
            echo "<h3>" . get_string("table_first_5", "local_kbi") . "</h3>";
            ...
            message::print_info(get_string("table_info_types", "local_kbi"));
            foreach ($lines[0] as $id => $line) {
                echo
                    "<fieldset><legend>" . get_string("table_edit_column", "local_kbi") .
                    ": <strong><em>{$id}</em></strong></legend>";
                $this->select_data($koperebielement, $id);
                echo "</fieldset>";
            }
        } else {
            message::print_warning(get_string("sql_no_rows", "local_kbi"));
            return false;
        }
    ...
    private function select_data($koperebielement, $collkey) {
        ...
        $types = [
            [
                "key" => "none",
                "value" => get_string("table_renderer_none", "local_kbi"),
            ], [
                "key" => "string",
                "value" => "...",
            ], [
                "key" => "number",
                "value" => get_string("table_renderer_number", "local_kbi"),
            ], [
                "key" => "userfullname",
                "value" => get_string("table_renderer_userfullname", "local_kbi"),
            ], [
                "key" => table_header_item::RENDERER_USERPHOTO,
                "value" => get_string("table_renderer_userphoto", "local_kbi"),
            ], [
                "key" => table_header_item::RENDERER_VISIBLE,
                "value" => get_string("table_renderer_visible", "local_kbi"),
            ], [
                "key" => table_header_item::RENDERER_STATUS,
                "value" => get_string("table_renderer_status", "local_kbi"),
            ], [
                "key" => table_header_item::RENDERER_DATE,
                "value" => get_string("table_renderer_date", "local_kbi"),
            ], [
                "key" => table_header_item::RENDERER_DATETIME,
                "value" => get_string("table_renderer_datetime", "local_kbi"),
            ], [
                "key" => table_header_item::RENDERER_TIME,
                "value" => get_string("table_renderer_seconds", "local_kbi"),
            ], [
                "key" => "translate",
                "value" => get_string("table_renderer_translate", "local_kbi"),
            ], [
                "key" => table_header_item::RENDERER_FILESIZE,
                "value" => get_string("table_renderer_filesize", "local_kbi"),
            ],
        ];
        ...
        $form->add_input(
            input_text::new_instance()
                ->set_title(get_string("table_col_title", "local_kbi"))
        ...
        $form->add_input(
            input_select::new_instance()
                ->set_title(get_string("table_renderer_title", "local_kbi"))
        ...
        $PAGE->requires->js_call_amd("local_kbi/table", "select", [$collkey, $changemustache]);
        ...
        $form->add_input(
            input_textarea::new_instance()
                ->set_title(get_string("table_renderer_mustache", "local_kbi"))
        ...
        $PAGE->requires->js_call_amd("local_kbi/load_ace", "getScript", ["columnmustache{$collkey}", "html", 3]);
    }
    ...
    public function preview($koperebielement) {
        ...
        if (!isset($koperebielement->info_obj["column"])) {
            return message::danger(get_string("table_column_not_configured", "local_kbi"));
        }
    ...
    public function preview_google($koperebielement) {
        ...
        return $OUTPUT->render_from_template("local_kbi/block_table_preview_google", [
