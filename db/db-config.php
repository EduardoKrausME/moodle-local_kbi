...
function reset_bi_reports() {
    ...
    set_config("theme_palette", "default", "local_kbi");

    $json = file_get_contents(__DIR__ . "/files/chart_pie_default.js");
    $json = str_replace("json = ", "", $json);
    set_config("chart_pie_default", $json, "local_kbi");

    $json = file_get_contents(__DIR__ . "/files/chart_column_default.js");
    $json = str_replace("json = ", "", $json);
    set_config("chart_column_default", $json, "local_kbi");

    $json = file_get_contents(__DIR__ . "/files/chart_area_default.js");
    $json = str_replace("json = ", "", $json);
    set_config("chart_area_default", $json, "local_kbi");

    $json = file_get_contents(__DIR__ . "/files/chart_line_default.js");
    $json = str_replace("json = ", "", $json);
    set_config("chart_line_default", $json, "local_kbi");

    try {
        $DB->delete_records("local_kbi_cat");
        $DB->delete_records("local_kbi_page");
        $DB->delete_records("local_kbi_block");
        $DB->delete_records("local_kbi_element");
    } catch (Exception $e) {
        return;
    }
    ...
    foreach ($pagefiles as $pagefile) {
        ...
        /** @var local_kbi_cat $category */
        $category = $DB->get_record("local_kbi_cat", ["refkey" => $page->category->refkey]);
        if (!$category) {
            $category = $DB->get_record("local_kbi_cat", ["title" => $page->category->title]);
            if (!$category) {
                $category = $page->category;
                $category->id = $DB->insert_record("local_kbi_cat", $category);
            }
        }

        $koperebipage->time = time();
        $koperebipage->cat_id = $category->id;
        $page->id = $DB->insert_record("local_kbi_page", $koperebipage);
        ...
        foreach ($page->blocks as $block) {
            ...
            $block->id = $DB->insert_record("local_kbi_block", $koperebiblock);
            ...
            foreach ($block->elements as $element) {
                ...
                $element->id = $DB->insert_record("local_kbi_element", $koperebielement);
            }
        }
    }
}
