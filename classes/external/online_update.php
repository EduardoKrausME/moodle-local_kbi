...
use local_kbi\local\vo\local_kbi_online;
...
class online_update extends external_api {
    ...
    public static function api($onlineid, $cachekey, $seconds) {
        ...
        /** @var local_kbi_online $online */
        $online = $DB->get_record("local_kbi_online", ["id" => $params["online_id"]]);
        if ($online) {
            $online->currenttime = time();
            $online->seconds = $online->seconds + $params["seconds"];

            $DB->update_record("local_kbi_online", $online);
        }
        return ["success" => true];
    }
}
