...
use local_kbi\local\block\area;
use local_kbi\local\block\column;
use local_kbi\local\block\html;
use local_kbi\local\block\i_type;
use local_kbi\local\block\info;
use local_kbi\local\block\line;
use local_kbi\local\block\maps;
use local_kbi\local\block\pie;
use local_kbi\local\block\table;
...
class type_block extends bi_all {
    ...
    public function select_type($blockid, $blocknum) {
        ...
        $block = $DB->get_record("local_kbi_block", ["id" => $blockid]);
        ...
        dashboard_util::add_breadcrumb(get_string("title", "local_kbi"),
        ...
        dashboard_util::add_breadcrumb(get_string("select_report_select_type", "local_kbi"));
        dashboard_util::start_page();

        echo '<div class="element-box">';
        echo "<h3>" . get_string("select_report_select_type_desc", "local_kbi") . "</h3>";
        ...
        foreach ($buttons as $button) {
            echo $OUTPUT->render_from_template("local_kbi/type_block-select_type", $button);
        }

        echo "</div>";
        dashboard_util::end_page();

        $PAGE->requires->js_call_amd("local_kbi/choose-block-type", "charts", []);
    }
    ...
    public static function type_block_edit_salvar($koperebielement, $koperebipage, i_type $block) {
        ...
        if (form::check_post()) {
            ...
            if (isset($koperebielement->id)) {
                $DB->update_record("local_kbi_element", $koperebielement);
                ...
                /** @var local_kbi_element $element */
                $element = $DB->get_record("local_kbi_element", ["block_id" => $blockid, "block_num" => $blocknum]);
                ...
                if ($element) {
                    ...
                    $koperebielement->id = $DB->insert_record("local_kbi_element", $koperebielement);
                }
            }
