...
use dml_exception;
use local_kbi\local\analise\access_analyze;
...
class core_hook_output {
    ...
    public static function before_footer_html_generation() {
        ...
        if (!isset($USER->koperebionline_id[$key])) {
            $lastip = local_kbi_getremoteaddr();

            $dataagent = access_analyze::agent();
            $dataip = local_kbi_iplookup_find_location($lastip);
            ...
            try {
                $koperebionlineid = $DB->insert_record("local_kbi_online", $koperebionline);
        ...
        if (isset($USER->koperebionline_id[$key])) {
            $PAGE->requires->js_call_amd("local_kbi/online", "init", [$USER->koperebionline_id[$key], $key]);
        }
        $PAGE->requires->js_call_amd("local_kbi/mod_koperebi", "init");
    }
}
