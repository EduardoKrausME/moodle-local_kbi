...
use local_kbi\form\extra_langs_changue_component;
...
class extra_langs extends bi_all {
    ...
    public function index() {
        ...
        $component = optional_param("component", "local_kbi", PARAM_TEXT);
        ...
        echo $OUTPUT->render_from_template("local_kbi/extra_langs_existinginlang_index", [
            "existinginlang" => $existinginlang,
            "component" => $component,
        ]);

        if ($component == "local_kbi") {

            if (form::check_post()) {
                $toolcustomlangcomponent = $DB->get_record("tool_customlang_components", ["name" => "local_kbi"]);
            ...
            for ($i = 0; $i < 80; $i++) {
                $identifier = "word_extra_" . substr("0{$i}", -2);
                $string = get_string($identifier, "local_kbi");
            ...
            echo $OUTPUT->render_from_template("local_kbi/extra_langs_custom_index", [
                "customs" => $customs,
                "component" => $component,
            ]);
