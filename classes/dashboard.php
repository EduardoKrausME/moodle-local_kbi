...
use local_kbi\local\block\i_type;
...
use local_kbi\local\util\preview_util;
use local_kbi\local\util\scss_util;
use local_kbi\local\util\string_util;
use local_kbi\local\vo\local_kbi_block;
use local_kbi\local\vo\local_kbi_cat;
use local_kbi\local\vo\local_kbi_element;
...
class dashboard extends bi_all {
    ...
    public function start() {
        ...
        dashboard_util::add_breadcrumb(get_string("title", "local_kbi"), "?classname=bi-dashboard&method=start");
        dashboard_util::start_page();

        $koperebicats = $DB->get_records("local_kbi_cat", null, "sortorder ASC");
        $categorys = [];

        /** @var local_kbi_cat $koperebicat */
        foreach ($koperebicats as $koperebicat) {

            $koperebipages = $DB->get_records("local_kbi_page", ["cat_id" => $koperebicat->id], "sortorder ASC");
        ...
        echo $OUTPUT->render_from_template("local_kbi/dashboard-start", [
            "categorys" => $categorys,
    ...
    public function edit() {
        ...
        if ($pageid) {
            /** @var local_kbi_page $page */
            $page = $DB->get_record("local_kbi_page", ["id" => $pageid]);
            header::notfound_null($page, get_string("page_not_found", "local_kbi"));
            $title = get_string("page_edit", "local_kbi", string_util::get_string($page->title));
            ...
            if (form::check_post() && isset($pagetitle[3])) {
                ...
                $DB->update_record("local_kbi_page", $page);
            ...
            $title = get_string("page_new_cat", "local_kbi");

            if (form::check_post() && isset($pagetitle[3])) {
                unset($page->id);

                $page->refkey = uniqid();
                $page->id = $DB->insert_record("local_kbi_page", $page);
                header::location("?classname=bi-dashboard&method=edit_page&page_id={$page->id}");
            }
        }
        ...
        $form->add_input(
            input_text::new_instance()
                ->set_title(get_string("page_name", "local_kbi"))
                ->set_name("page_title")
        ...
        $form->add_input(
            input_textarea::new_instance()
                ->set_title(get_string("page_description", "local_kbi"))
                ->set_name("page_description")
                ->set_value($page->description)
        );

        if ($pageid) {
            $form->create_submit_input(get_string("save", "local_kbi"), "button");
        } else {
            $form->create_submit_input(get_string("create", "local_kbi"), "button");
        }
    ...
    public function edit_cat() {
        ...
        if ($catid) {
            /** @var local_kbi_cat $cat */
            $cat = $DB->get_record("local_kbi_cat", ["id" => $catid]);
            header::notfound_null($cat, get_string("cat_not_found", "local_kbi"));
            $title = get_string("cat_edit", "local_kbi", $cat->title);

            if (form::check_post() && isset($cattitle[3])) {
                $cat->title = $cattitle;
                $cat->description = $catdescription;

                $DB->update_record("local_kbi_cat", $cat);
            ...
            $title = get_string("cat_new", "local_kbi");

            if (form::check_post() && isset($cattitle[3])) {
                unset($cat->id);

                $cat->refkey = uniqid();
                $cat->id = $DB->insert_record("local_kbi_cat", $cat);
                header::location("?classname=bi-dashboard&method=start");
            }
        }
        ...
        $form->add_input(
            input_text::new_instance()
                ->set_title(get_string("cat_name", "local_kbi"))
                ->set_name("cat_title")
                ->set_value($cat->title)
                ->set_required()
        );
        ...
        $form->add_input(
            input_textarea::new_instance()
                ->set_title(get_string("cat_description", "local_kbi"))
    ...
    public function edit_page() {
        ...
        header::notfound_null($koperebipage, get_string("page_not_found", "local_kbi"));
        ...
        echo $OUTPUT->render_from_template("local_kbi/dashboard-topo-edit-page",
        ...
        echo "<div>" . get_string("page_new_sequence", "local_kbi") . "</div>";
        ...
        foreach ($koperebiblocks as $koperebiblock) {
            ...
            echo $OUTPUT->render_from_template("local_kbi/dashboard-dialog-confirm-block",
        ...
        $PAGE->requires->strings_for_js(["block_add"], "local_kbi");
        $PAGE->requires->strings_for_js(["close"], "admin");
        $PAGE->requires->js_call_amd("local_kbi/page-edit_page", "page_sortable", [$koperebipage->id]);
        $PAGE->requires->js_call_amd("local_kbi/page-edit_page", "page_blocks", [$koperebipage->id]);
    ...
    public function preview() {
        ...
        if (isset($USER->editing) && $USER->editing) {
            $editbooton = has_capability('moodle/site:config', $context) ?
                button::add(get_string("page_edit", "local_kbi"),
                    "?classname=bi-dashboard&method=edit_page&page_id={$koperebipage->id}",
    ...
    public function type_block_select_type() {
        ...
        /** @var local_kbi_element $koperebielement */
        $koperebielement = $DB->get_record("local_kbi_element", ["block_id" => $blockid, "block_num" => $blocknum]);
        if ($koperebielement) {
    ...
    public function type_block_preview() {
        ...
        /** @var local_kbi_element $koperebielement */
        $koperebielement = $DB->get_record("local_kbi_element", ["id" => $elementid]);
        header::notfound_null($koperebielement, get_string("item_not_found", "local_kbi"));

        $koperebielement->info_obj = @json_decode($koperebielement->info, true);

        /** @var local_kbi_block $block */
        $block = $DB->get_record("local_kbi_block", ["id" => $koperebielement->block_id]);
        header::notfound_null($block, get_string("block_not_found", "local_kbi"));

        /** @var local_kbi_page $page */
        $page = $DB->get_record("local_kbi_page", ["id" => $block->page_id]);
        header::notfound_null($page, get_string("page_not_found", "local_kbi"));

        dashboard_util::add_breadcrumb(get_string("title", "local_kbi"), "?classname=bi-dashboard&method=start");
        dashboard_util::add_breadcrumb(string_util::get_string($page->title),
            "?classname=bi-dashboard&method=edit_page&page_id={$page->id}");
        dashboard_util::add_breadcrumb(get_string("report_preview", "local_kbi"));
        dashboard_util::start_page();
        ...
        $class = "\\local_kbi\\local\\block\\{$koperebielement->type}";
        if (class_exists($class)) {
            /** @var i_type $block */
            $block = new $class();
            echo $block->preview($koperebielement);
        } else {
            message::print_danger(get_string("block_not_found", "local_kbi"));
        }
    ...
    public function type_block_edit() {
        ...
        /** @var local_kbi_block $koperebiblock */
        $koperebiblock = $DB->get_record("local_kbi_block", ["id" => $koperebielement->block_id]);
        header::notfound_null($koperebiblock, get_string("block_not_found", "local_kbi"));

        /** @var local_kbi_page $koperebipage */
        $koperebipage = $DB->get_record("local_kbi_page", ["id" => $koperebiblock->page_id]);
        header::notfound_null($koperebipage, get_string("page_not_found", "local_kbi"));

        /** @var i_type $class */
        $class = "\\local_kbi\\local\\block\\{$koperebielement->type}";
        if (!class_exists($class)) {
            message::print_danger(get_string("blocktype_not_found", "local_kbi"));
        }
        ...
        if ($elementid) {
            ...
            $blockname = "\\local_kbi\\local\\block\\{$koperebielement->type}";
            ...
            dashboard_util::add_breadcrumb(get_string("report_new", "local_kbi", $class::get_name()));
        }
        dashboard_util::start_page();
        ...
        if (isset($koperebielement->id)) {
            echo "<a href='?classname=bi-dashboard&method=type_block_preview&item_id={$koperebielement->id}'
                     class='btn btn-primary' target='_blank'>" . get_string("report_preview", "local_kbi") . "</a>";
        }

        $form = new form("?{$_SERVER["QUERY_STRING"]}");
        $form->add_input(
            input_text::new_instance()
                ->set_title(get_string("report_title", "local_kbi"))
                ->set_name("title")
                ->set_value($koperebielement->title));

        $form->add_input(
            input_select::new_instance()
                ->set_title(get_string("block_theme", "local_kbi"))
                ->set_name("theme")
                ->set_value(@$koperebielement->theme)
                ->set_values([
                    ["key" => "light", "value" => get_string("block_theme_light", "local_kbi")],
                    ["key" => "dark", "value" => get_string("block_theme_dark", "local_kbi")],
                    ["key" => "blue", "value" => get_string("block_theme_blue", "local_kbi")],
                    ["key" => "green", "value" => get_string("block_theme_green", "local_kbi")],
                    ["key" => "orange", "value" => get_string("block_theme_orange", "local_kbi")],
                    ["key" => "pink", "value" => get_string("block_theme_pink", "local_kbi")],
                ]));
        ...
        if ($block->is_edit_columns()) {
            $form->create_submit_input(get_string("report_save", "local_kbi"));
        } else {
            $form->create_submit_input(get_string("save", "local_kbi"));
        }
    ...
    public function type_block_edit_columns() {
        ...
        $blockclass = "\\local_kbi\\local\\block\\{$koperebielement->type}";
        ...
        echo "<div class='element-box'>";
        button::edit(get_string("return_edit", "local_kbi"),
    ...
    public function type_block_delete() {
        ...
        $DB->delete_records("local_kbi_element", ["id" => $koperebielement->id]);
    ...
    public function delete_block() {
        ...
        /** @var local_kbi_block $koperebiblock */
        $koperebiblock = $DB->get_record("local_kbi_block", ["id" => $blockid]);
        if (!$koperebiblock) {
            header::location("?classname=bi-dashboard&method=start");
        }

        $DB->delete_records("local_kbi_block", ["id" => $blockid]);
        $DB->delete_records("local_kbi_element", ["block_id" => $blockid]);
