...
class data_export {
    ...
    public function json() {
        ...
        $string = [];
        if (file_exists(__DIR__ . "/../lang/pt_br/local_kbi.php")) {
            require_once(__DIR__ . "/../lang/pt_br/local_kbi.php");
        } else {
            require_once(__DIR__ . "/../lang/en/local_kbi.php");
        }
        $stringbi = $string;
        ...
        $koperebicat = $DB->get_record("local_kbi_cat", ["id" => $koperebipage->cat_id]);
        ...
        $koperebiblocks = $DB->get_records("local_kbi_block", ["page_id" => $koperebipage->id]);
        /** @var local_kbi_block $koperebiblock */
        foreach ($koperebiblocks as $koperebiblock) {
            $elements = [];

            $koperebielements = $DB->get_records("local_kbi_element", ["block_id" => $koperebiblock->id]);
            ...
            foreach ($koperebielements as $koperebielement) {
                ...
                if (isset($infos->column->name)) {
                    ...
                    foreach ($infos->column->name as $key => $string) {
                        ...
                        if ($key == "firstname") {
                            $string = "lang::u_fullname::local_kbi";
        ...
        if ($this->missingstrings1) {
            $data->missingstrings1 = get_string("missingstrings1", "local_kbi", implode("\n", $this->missingstrings1));
        }
        if ($this->missingstrings2) {
            $data->missingstrings2 = get_string("missingstrings2", "local_kbi", implode("\n", $this->missingstrings2));
        }
