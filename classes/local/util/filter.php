...
use local_kbi\local\vo\external_report;
...
class filter {
    ...
    public static function create_filter_page($pageid) {
        ...
        $sql = "
            SELECT e.id, commandsql
              FROM {local_kbi_element} e
              JOIN {local_kbi_block}   b ON b.id = e.block_id
    ...
    private static function create_filter_course($course, $paramsurl) {
        ...
        $data = [
            ...
            "url_click" => local_kdashboard_makeurl($classname, $method, $paramsurl),
        ];
        $PAGE->requires->js_call_amd("local_kbi/filter_course", "init");
        return $OUTPUT->render_from_template('local_kbi/filter-course', $data);
    }
    ...
    private static function create_filter_user($user, $paramsurl) {
        ...
        $PAGE->requires->js_call_amd("local_kbi/filter_user", "init");
        return $OUTPUT->render_from_template('local_kbi/filter-user', $data);
    }
